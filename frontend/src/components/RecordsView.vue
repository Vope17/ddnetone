...
// 修改 src/components/RecordsView.vue 的 categoryStats

const categoryStats = computed(() => {
  const list = mapsInCurrentCategory.value;

  // 1. 計算目前獲得分數 (只算已完成的)
  const currentScore = list.reduce((acc, m) => {
    return acc + (m.status === 2 ? (m.points || 0) : 0);
  }, 0);


  // 2. ★ 新增：計算該分類「總分」 (所有地圖的 points 加總) ★
  const totalScore = list.reduce((acc, m) => {
    return acc + (m.points || 0);
  }, 0);
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="flex items-baseline gap-1">
                ...
                <span class="text-xs text-gray-600 font-mono ml-1">PTS</span>
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-between items-start mb-6 mt-2">
            ...
            <div class="text-right mt-8">
              <span :class="['font-mono text-2xl font-bold tracking-tight',
                ...
                map.status === 2 ? 'text-white drop-shadow-[0_0_5px_rgba(6,182,212,0.5)]' : 'text-gray-600']">
                {{ map.points }}
              </span>
              <span
